...
class DynamicItem(get_tree_item_model()):
    ...
    def __init__(
        ...
        self.permissions = SimpleLazyObject(
            lambda: self._clean_permissions(access_by_perms)
        )
    ...
    def _clean_permissions(self, permissions):
        if hasattr(self, "clean_dynamic_permissions"):
            return self.clean_dynamic_permissions(permissions)
        return clean_permissions(permissions)
